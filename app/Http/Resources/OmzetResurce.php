...
class OmzetResurce extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'harga_jual' => $this->transaksi->harga_jual ?? 0,
            'total_tagihan' => $this->transaksi->jumlah_beli * $this->transaksi->harga_jual ?? 0,
            'supplier' => $this->transaksi->Suppliers->nama ?? '-',
            'harga_beli' => $this->transaksi->harga_beli ?? 0,
            'total' => $this->transaksi->harga_beli * $this->transaksi->jumlah_beli ?? 0,
            ...
            'harga_beli_ppn' => $ppnBeli != 0 ? $ppnBeli : 0,
            'margin_ppn' => $ppnMargin,
            'margin' => ($this->transaksi->harga_jual * $this->transaksi->jumlah_beli) - ($this->transaksi->harga_beli * $this->transaksi->jumlah_beli) ?? 0,
            'margin_cek' => $ppnCekMargin,
            'satuan_standar' => $this->transaksi->Barang->Satuan->nama_satuan ?? '-',
            'beli' => ($this->transaksi->satuan_beli == $this->transaksi->Barang->Satuan->nama_satuan) ? $this->transaksi->harga_beli : $this->transaksi->harga_beli / $this->transaksi->Barang->value ?? 0,
            'jual' => ($this->transaksi->satuan_beli == $this->transaksi->Barang->Satuan->nama_satuan) ? $this->transaksi->harga_jual : $this->transaksi->harga_jual / $this->transaksi->Barang->value ?? 0
        ];
    }
}
