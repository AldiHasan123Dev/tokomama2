...
class SuratJalanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['no'] = $no;
        $data['nomor_surat'] = sprintf('%03d', $no) . '/SJ/TM_RAS1-' . $month_roman . '/' . date('Y', strtotime($request->tgl_sj));
    ...
    public function store_bm(Request $request)
    {
        ...
        $month_roman = $roman_numerals[$month_number];
        $no_bm = sprintf('%03d', $no) . '/BM/TM_RAS1-' . $month_roman . '/' . date('Y', strtotime($request->tgl));
        for ($i = 0; $i < count($request->barang); $i++) {
            // dd($request->barang);
